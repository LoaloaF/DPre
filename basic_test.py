...
import glob
import sys, os
from DPre import Samples, Targets, preset_targets, config, plot_color_legend




# t = preset_targets('human')
# plot_color_legend(['blabla'], (('gt', .4,.57,.1), ), 'gee.pdf')
...
# preset_targets('m mesoderm')
# t = Targets(expression=fn, 
#             markergenes=dirr,
#             name='early embryonic fates', 
#             species='mouse',
#             override_namematcher=True)
# markergenes=dirr, override_namematcher=True, ignore_down_mgs=False)
...
# os.chdir('test_data/deseq2/')
# for f in glob.glob('*.tsv'):
#     diff = pd.read_csv(f, sep='\t', index_col=0)
#     print(diff)
#     sig = diff['padj'] <.05
#     up = diff['log2FoldChange'] >0
#     down = diff['log2FoldChange'] <0
#     up_genes = sig.index[sig & up]
#     dn_genes = sig.index[sig & down]

#     pd.DataFrame(False, index=up_genes, columns=['hm']).to_csv('gls/up/'+f, sep='\t')
#     pd.DataFrame(False, index=dn_genes, columns=['hm']).to_csv('gls/down/'+f, sep='\t')

c = Samples(
            # diff_genes = ('test_data/deseq2/gls/up', 'test_data/deseq2/gls/down'),
            # diff_genes = 'test_data/deseq2/',
            expression = 'test_data/genes_ntc_expression.tsv', 
            ctrl = 'none', 
            name = 'epigenetic inhihbitor combinations',
            override_namematcher=True)
# c = c.slice_elements(['none'])
...
t = preset_targets('m embryonic')
t.gene_similarity_heatmap(c, filename='testfile.png', specific_genes=genes, differential=True)
#                                 differential = True,
#                                 proportional = True, 
#                                 gene_number = 20,
