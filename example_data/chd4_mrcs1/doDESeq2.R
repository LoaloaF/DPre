...
for (i in 1:dim(contrasts)[1]) {
  ...
  c1 = contrasts[i,]$c1
  c2 = contrasts[i,]$c2
  res = results(dds, contrast=c("shrna", c2, c1))
  
  dir.create("./deseq_res")
  write.table(res, file=paste("deseq_res/", row.names(contrasts[i,]), ".all.tsv", sep=""), sep="\t", col.names=NA)
  ...
  dn = pres[pres$log2FoldChange < 0 ,] 
  up = pres[pres$log2FoldChange > -0,] 
  ...
  # write.table(pres, file=paste("deseq_res/", row.names(contrasts[i,]), ".all.tsv", sep=""), sep="\t", col.names=NA)
